...
def string_to_boolean(
    col: pl.Expr, true_values: list[str], false_values: list[str]
) -> pl.Expr:
...
def trim_string_columns(df: pl.LazyFrame) -> pl.LazyFrame:
    ...
    return df.with_columns(
        *[pl.col(col_name).str.strip_chars() for col_name in string_cols]
    )
...
def nullify_string_columns(df: pl.LazyFrame) -> pl.LazyFrame:
    ...
    string_cols = get_string_column_names(df)
    return df.with_columns(
        *[pl.col(col_name).replace("", None) for col_name in string_cols]
    )
